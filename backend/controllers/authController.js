...
const register = async (req, res) => {
    ...
    try {
        ...
        if (e.code === "23505") return res.status(409).json({ message: "Username atau email sudah terdaftar." });
...
const login = async (req, res) => {
    ...
    try {
        ...
        // Validate Password
        const isMatch = await AuthService.validatePassword(password, user.kata_sandi);
        if (!isMatch) return res.status(401).json({ message: "Password salah." });

        // Generate Token (Logic remains here as it's HTTP/Auth related)
